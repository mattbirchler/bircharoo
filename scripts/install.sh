...
# Usage: scripts/install.sh [vault-path]   (default: ~/Obsidian/Birchler-alt, the dev vault)
...
if command -v obsidian >/dev/null 2>&1; then
  obsidian vault="$VAULT_NAME" eval code="app.customCss.readThemes().then(()=>app.customCss.setTheme('Bircharoo')).then(()=>new Promise(r=>setTimeout(r,800))).then(()=>'reloaded')" >/dev/null 2>&1 || true
fi
